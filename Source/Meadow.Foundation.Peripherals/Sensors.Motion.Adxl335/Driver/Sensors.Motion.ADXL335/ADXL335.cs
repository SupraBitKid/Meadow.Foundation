...
namespace Meadow.Foundation.Sensors.Motion
{
    ...
    public class ADXL335
    {
        ...
        {
            X = ( await _xPort.Read() * SupplyVoltage - _zeroGVoltage) / XVoltsPerG;
            Y = ( await _yPort.Read() * SupplyVoltage - _zeroGVoltage) / YVoltsPerG;
            Z = ( await _zPort.Read() * SupplyVoltage - _zeroGVoltage) / ZVoltsPerG;
        ...
        {
			return await Task<Vector>.FromResult( new Vector( _xPort.Read().Result, _yPort.Read().Result, _zPort.Read().Result ) );
        }
